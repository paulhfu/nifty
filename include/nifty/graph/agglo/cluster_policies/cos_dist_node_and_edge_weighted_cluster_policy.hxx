...
>
class CosineDistNodeAndEdgeWeightedClusterPolicy{

    typedef CosineDistNodeAndEdgeWeightedClusterPolicy<
        GRAPH, ENABLE_UCM
    > SelfType;
...
public:
    ...
    template<
        ...
        class NODE_SIZES
    >
    CosineDistNodeAndEdgeWeightedClusterPolicy(const GraphType &,
...
inline CosineDistNodeAndEdgeWeightedClusterPolicy<GRAPH, ENABLE_UCM>::
CosineDistNodeAndEdgeWeightedClusterPolicy(
...
CosineDistNodeAndEdgeWeightedClusterPolicy<GRAPH, ENABLE_UCM>::
...
template<class GRAPH, bool ENABLE_UCM>
inline typename CosineDistNodeAndEdgeWeightedClusterPolicy<GRAPH, ENABLE_UCM>::EdgeContractionGraphType &
CosineDistNodeAndEdgeWeightedClusterPolicy<GRAPH, ENABLE_UCM>::
...
mergeNodes(
    ...
    for(auto c=0; c<nChannels_; ++c){
        featA[c] = sizeA*featA[c] + sizeD*featD[c];
    }
	for(auto c=0; c<nChannels_; ++c){
		norm += std::pow(featA[c], 2);
	}
	norm = std::sqrt(norm);
	for(auto c=0; c<nChannels_; ++c){
		featA[c] /= norm;
