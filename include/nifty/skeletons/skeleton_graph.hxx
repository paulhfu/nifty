...
namespace skeletons {
    ...
    public:
        ...
        typedef nifty::array::StaticArray<int64_t,DIM> Coordinate;
        typedef std::vector<Coordinate> Coordinates;

        SkeletonGraph(
                marray::View<MarkerType,true> skeleton,
                double anisotropy,
                unsigned anisotropicDimension = 0);
        ...
        const Coordinate & nodeCoordinate(const int64_t nodeId) const{
            return (nodeId < terminalNodes_.size()) ? terminalNodes_[nodeId] : branchNodes_[nodeId - terminalNodes_.size()];
        }


    private:

        void findNodes(marray::View<int64_t> & nodeMap);
        void growToBranchNodes(marray::View<bool> & visited, const marray::View<int64_t> & nodeMap);
        void constructGraph(   marray::View<bool> & visited, const marray::View<int64_t> & nodeMap);
        ...
        inline void checkAdjacencyBoxWithVisited(
                const Coordinate & coord,
                const marray::View<bool> & visited,
                Coordinates & nextCoordinates) {

        }

        marray::View<MarkerType,true> skeleton_;
        double anisotropy_;
    ...
    template<unsigned DIM, typename MARKER_TYPE>
    SkeletonGraph<DIM,MARKER_TYPE>::SkeletonGraph(
            marray::View<MARKER_TYPE, true> skeleton,
            double anisotropy,
            unsigned anisotropicDimension) :
        skeleton_(skeleton),
    ...
    template<unsigned DIM, typename MARKER_TYPE>
    void SkeletonGraph<DIM, MARKER_TYPE>::findNodes(marray::View<int64_t> & nodeMap) {
    ...
    void SkeletonGraph<DIM, MARKER_TYPE>::growToBranchNodes(
        ...
        for(size_t u = 0; u < terminalNodes_.size(); ++u) {
            ...
            while(true) {
                ...
                else if(nextCoords.size() > 1) {
                    auto v      = nodeMap(coord.asStdArray());
                    auto edgeId = graph_.insertEdge(u, v);
                    NIFTY_ASSERT_OP(edgeCoordinates_.size(),==,edgeId);
    ...
    template<unsigned DIM, typename MARKER_TYPE>
    void SkeletonGraph<DIM, MARKER_TYPE>::constructGraph(
            marray::View<bool> & visited,
            const marray::View<int64_t> & nodeMap) {
